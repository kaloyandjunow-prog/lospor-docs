...
- **Separate Infusions entry point (web).** Starting an infusion on web no longer requires picking a drug and then choosing "Bolus" vs "Infusion" вЂ” there's now a dedicated Infusions row, matching how the mobile app has always separated Drug/Infusion/Fluid/Agent entry.
...
- **Wording.** Documentation now uses вЂњde-identified/pseudonymisedвЂќ and describes the OMOP export as partial/OMOP-inspired until full concept mapping is complete.
...
- **OMOP export: drug events from event log.** Drug exposure in the OMOP CDM export now reads from the `CaseEvent` table (type=drug, status=active) вЂ” the canonical append-only event log вЂ” instead of parsing the legacy `keyEvents` JSON blob. ATC codes appear in `drug_source_concept_id`.
...
- **Field-level audit trail.** Every preoperative and postoperative field change is now recorded individually вЂ” what changed, from what value, to what value, by whom, and when.
...
- **Clinical data is now stored as queryable database rows.** Diagnoses, procedures,
  ...
  (positions, techniques, airway, ventilation, handover) вЂ” previously held only as
  JSON вЂ” are now also written as proper rows, making research and data export much
...
- **Finalised cases are now fully locked** вЂ” intraoperative entries can't be edited or deleted once a case is finalised.
...
- **Department-head view scoped to your own institution** вЂ” a Head of Department now sees only cases from their institution, not every case in the system.
...
- **Infusion rate changes display correctly** вЂ” the chart and pills show the right rate before and after each change.
...
### Mobile improvements
- **Settings redesign** вЂ” settings is now two-level: a Profile screen (name, institution, edit institution from a DB list) and a Settings screen (UI: theme/language/preop layout; Automation: auto-fill vitals/BP/HR/background refresh; Privacy & Data: policy/terms/about/export/delete). Admin console visible to admins only. Sign out is a separate persistent button.
- **Inline procedure and diagnosis search** вЂ” mobile search fields now use inline dropdown autocomplete instead of full-screen sheets. Procedure results display the clinical group as the primary label and the code and domain below it, matching the web app.
- **AI monitor scan** вЂ” fixed on native Android; camera images now correctly pass base64 to the vitals-scan endpoint.
- **AI advisor removed from case summary** вЂ” the AI pre-operative advisor button is available only in the preop form. It was incorrectly appearing on the case summary screen.
- **Case status chain completed** вЂ” mobile dashboard and case summary now reflect the full seven-step status chain: Draft в†’ In Consultation в†’ Awaiting Allocation в†’ In Theatre в†’ Awaiting Post-op в†’ Awaiting Review в†’ Case Finished.

### Lab scan improvements
- **Library-anchored extraction** вЂ” the AI lab scan now only returns tests from the LOSPOR catalogue. Unknown or phantom test names (e.g. "absolute leucocyte count") are silently discarded server-side.
- **Normalised units** вЂ” all extracted results are mapped to canonical units: Hb in g/L, Hct as a decimal ratio, glucose in mmol/L, and so on. Unit normalisation is enforced server-side regardless of how the value appears in the source image.
- **Custom lab results removed** вЂ” free-form custom lab entries have been removed. All results must come from the catalogue, ensuring consistent units and reference ranges.

### PWA fixes
- **Timetable autosize on PWA** вЂ” vital-sign input fields, blood pressure popup, and drug dose controls in the intraoperative timetable now adapt to the browser window width. Previously they overflowed off the right edge of the screen.
- **Dark mode colour-scheme error fixed** вЂ” `darkMode: "class"` is now set in the Tailwind config, preventing a `Cannot manually set color scheme` console error on the PWA.

### Legal
- **Privacy Policy updated to v1.1** вЂ” sub-processors section now explicitly covers Mistral AI image processing for lab scan and monitor scan. Effective date updated to June 2026.
- **Terms of Service updated to v1.1** вЂ” new section 3a documents user obligations when using AI image scanning features. Effective date updated to June 2026.
- **AGPL-3.0 LICENSE added to mobile app** вЂ” `lospor-mobile/LICENSE` created. Copyright (C) 2026 Kaloyan Dzhunov.
...
### Features
- **Full Bulgarian UI translation** вЂ” every user-visible string in the interface now adapts to the selected language. All previously hardcoded English labels, section headers, button text, error messages, and status indicators across the admin panel, case entry wizard, register page, settings, guided tour, and preoperative form have been converted to translatable keys. Switching to Bulgarian in Settings в†’ Language now translates the entire app.
- **Vercel Analytics** вЂ” anonymous page-view tracking added. No personal data is collected.

### Security / compliance
- **AI disclaimer corrected** вЂ” the AI advisor no longer uses "clinical decision support" language. The disclaimer now clearly states that the output is an informational summary, does not constitute clinical advice, and that the responsible anaesthesiologist retains full clinical responsibility.
- **Lab scan upload warning strengthened** вЂ” the GDPR notice above the upload button now explicitly instructs users to crop patient names, date of birth, ID numbers, and other identifiers out of the image before uploading.
- **PII detection best-effort notice** вЂ” the Privacy Policy now clarifies that automatic pattern detection is best-effort. Users remain responsible for not entering patient-identifiable information in free-text fields.
...
### Fixes
- **Terms and Privacy links not opening when logged in** вЂ” clicking Terms or Privacy in the app footer redirected back to the dashboard. Fixed.
...
### Features
- **30-minute review window** вЂ” submitting the postoperative form now opens a 30-minute review period instead of immediately locking the case. A countdown banner is visible at every step. Navigate back to preop, intraop, or postop to correct any data. The case auto-closes when the timer expires or you click **Close Now**. The timer persists if you leave and return to the page.
- **Expanded lab catalogue** вЂ” the preoperative Labs section now includes 100+ perioperative-relevant tests across nine categories: Haematology, Coagulation, Electrolytes, Biochemistry, Liver, Cardiac, Blood Gas, Thyroid, and Inflammatory/Other. Tests are shown in collapsible category rows.
- **Lab reference ranges** вЂ” each entered result is compared to a reference interval and flagged as normal (green) or out of range (amber). No clinical action is implied; the flag is informational only.
- **Lab search** вЂ” type in the search box above the catalogue to filter tests instantly.
- **AI lab scan** вЂ” click **Scan lab report** to upload a photo of a printed lab result. Mistral AI reads the image and extracts test names, values, and units. A preview panel shows the extracted results; select which ones to add. A GDPR notice is shown above the upload button at all times.
- **HOD access restricted to own institution** вЂ” Heads of department can view and edit only cases belonging to members of their own institution. Case transfers are also restricted to within-institution recipients. Admin access remains global.

### Fixes
- **Autosave error on case reopen** вЂ” returning to the intraop form after navigating away caused a validation error and autosave failure. Fixed.
- **Postop data blank on reopen** вЂ” reopening a case that had already been submitted through postop showed empty postop fields. All data is now restored.
- **Review window resets on navigation** вЂ” leaving and returning to the summary page restarted the 30-minute timer from scratch. The timer now resumes from the correct remaining time.
- **Parallel fluid lane disappears** вЂ” inline-discontinuing one of two same-category parallel fluids caused the discontinued lane to vanish from the timetable. Fixed.
- **Lab results cut off in print** вЂ” entering more than 12 or so lab results caused them to overflow and be clipped in the printed protocol. The summary now uses a multi-column layout with a compact font so up to ~40 results fit on the page.
- **Summary cards too narrow on first open** вЂ” the printable summary was narrower than expected on the first open during case entry. Fixed.

---

## [0.3.0] вЂ” 2026-05-21

### GDPR вЂ” Data minimisation
- **Removed staff names** вЂ” surgeon, anaesthesiologist, and nurse name fields removed from the preoperative form. Replaced by a free-text **Team notes** field with a privacy warning.
- **Removed exact surgery date** вЂ” the date field is replaced by a month/year selector. No calendar date is stored.
- **Anonymous case codes** вЂ” format changed from `DDMMYYYY-NN` to `YYYY-NNNN` (e.g. `2026-0001`).
- **Patient identity never stored** вЂ” the printable protocol leaves identity fields blank for hand-writing after printing. The print-time name/ID dialog has been removed.
- **Institution decoupled from Case** вЂ” institution is now stored on the user account only.

### GDPR вЂ” Consent and transparency
- **Consent screen** вЂ” shown on first login; must be accepted before using the app.
- **Terms checkbox on registration** вЂ” new accounts must accept the Terms of Use and Medical Disclaimer.
- **Privacy Policy page** (`/privacy`) and **Terms of Service page** (`/terms`) вЂ” accessible without login.
- **Footer links** вЂ” Terms В· Privacy В· Open source В· AGPL-3.0 added throughout the app.
...
- **Constant-time login check** вЂ” prevents email enumeration via response timing.
- **Last login tracking** вЂ” displayed in Settings в†’ Privacy & Data.
- **Soft-delete** вЂ” deleted accounts cannot log in.
- **Server-side PII detection** вЂ” free-text fields are checked for EGN, long digit sequences, date patterns, email addresses, and name patterns. Returns a clear 400 error and logs to the audit trail.

### AI advisor
- **Migrated to Mistral La Plateforme (EU)** вЂ” EU-hosted inference with GDPR DPA. Groq removed.
- **Free-text fields stripped** вЂ” only structured clinical fields are sent; notes and free-text are never forwarded.
- **Opt-in per case** вЂ” disabled by default; enabled via a toggle in the preop form.

### Features
- **Settings в†’ Privacy & Data** вЂ” last login, data export, account deletion.
- **GuardedTextarea** вЂ” live character counter and blur warning for EGN/MRN patterns on free-text inputs.
- **Admin / HOD case access** вЂ” admins and heads of department can view and edit cases owned by any member.

### Fixes
- **Timetable timezone** вЂ” times were shifting by the UTC offset on every reload; fixed by using UTC methods when reading stored times.
- **Autosave schema coercion** вЂ” HTML inputs return strings; API schemas now coerce string values, preventing Zod 400 errors mid-typing.
- **Autosave no longer locks cases** вЂ” postop autosave no longer promotes the case to COMPLETE; only the final submit button does.
- **PDF empty 3rd page** вЂ” footer text overflow fixed.
