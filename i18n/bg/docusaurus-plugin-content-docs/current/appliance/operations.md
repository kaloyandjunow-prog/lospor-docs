...
- Ако тайните са се променили, съхранете ги отново: **Поддръжка → Съхранение на тайните** (или `sudo losporctl secrets escrow /media/usb`).
- **Поддръжка → Смяна на данните за достъп** (или `sudo losporctl secrets rotate`,
  след това `sudo losporctl secrets commit`), в спокоен период: всички влизат
  отново. След това съхранете тайните отново в **Поддръжка → Съхранение на тайните**.
...
какво изисква връщането след миграция. [Архивиране и възстановяване](backup-restore.md)
...
за наблюдение. `version`, `backup list`, `backup offhost state`, `config show`,
`config advanced` и `host state` също приемат `--json` и отпечатват по един
обект със `schemaVersion`; всяка команда, която променя системата, отказва
`--json` с изходен код 2. `support-bundle create` записва файл за поддръжката на LOSPOR
...
[Мрежови и TLS граници](network-boundaries.md), а за управлявания staged
import, go-live, rollback и finalize —
[Импорт на терминология](terminology-import.md). И двете процедури са
...
файл. Вижте [Наблюдение чрез Status](status-monitor.md) за процедурите за
...
**Поддръжка → Смяна на данните за достъп** в Status прави точно това чрез
агента на сървъра, след като администраторът отметне потвърждението и въведе
отново паролата. Сменят се ключът за сесиите, токените на работните процеси,
токените на Status и двете пароли за базата данни; ключовете за идентичност на
пациентите, псевдонимите, шифроването, манифеста на архивите и печатите,
ключът за MFA и входовете в Status никога не се сменят. Смяна, която вече чака
от конзолата, прекъсната смяна или смяна, чиято отмяна не може да бъде доказана,
се оставя на болничния ИТ екип в конзолата със `state`. Процедурата в конзолата:
...
[Смяна на оперативните данни за достъп](secret-rotation.md) за отделните
...
[Предоставяне на Hospital акаунти](account-provisioning.md).
